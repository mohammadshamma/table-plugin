...
- **server.py** — declares the 16 MCP tools (`table_create/insert/join/group_by/run_sql/schema/list/drop/import_csv` + `table_job_create/claim/submit/status` + `table_inspect_start/stop` + `table_version`) and dispatches them to functions imported from `table_tool.py` (no subprocess per call). `table_version` is answered before the DB is resolved (a version check must not create a session db); it compares `LOADED_BUILD` — a digest of `server.py` + `table_tool.py` frozen at **import** — against the same files on disk, so `stale: true` means Antigravity is still running pre-edit code and must be restarted. Also owns three subsystems that live only here:
...
- **table_tool.py** — the actual SQLite operations (`op_create_table`, `op_insert`, `op_import_csv`, `op_join`, `op_group_by`, `op_query`, `op_schema`, `op_tables`, `op_drop`), all returning JSON. Also works standalone as a CLI (`table_tool.py create-table <db> <table> <json>` …), which is how `test_table_tool.py` exercises it. `op_import_csv` infers INTEGER/REAL/TEXT column types deterministically in code (`infer_column_types` — whole-data scan, regex-gated; never the LLM) and runs the exists-check + CREATE + inserts in one transaction. Also owns the read-only job helpers shared with the inspector — `JOBS_TABLE`, `RESERVED_TASK_COLUMNS`, `count_task_statuses`, `list_job_tables` (server.py re-imports the first two) — so the web view and `table_job_status` can never disagree about a job's counts. `op_query` (`table_run_sql`) runs behind `QueueGuard`, a SQLite authorizer that makes the `_table_jobs` registry and a job table's reserved `_task_*`/`result` columns read-only: a job's guarantees hold only while task state moves through the `table_job_*` tools, and arbitrary SQL could otherwise forge results or disable the claim cap without ever touching the DB file. It also denies `ALTER`/`CREATE TRIGGER` on protected tables and `PRAGMA writable_schema` (the schema-side routes to the same end). Protection is read from the registry per call, so deregistering a job releases it; the guard is installed only on `op_query`'s connection, never on `job_connect`'s.
